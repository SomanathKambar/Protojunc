...
fun startDiscovery() {
    try {
        ...
        val serviceInfo = ServiceInfo.create("_directo._tcp.local.", "Directo-Signaling", 8080, "Directo Signaling Server")
        jmdns.registerService(serviceInfo)
        println("[SERVER] [mDNS] Discovery active: _directo._tcp.local.")
    } catch (e: Exception) {
...
fun main() {
    println("[SERVER] Starting Directo Signaling Server on port 8080...")
    startDiscovery()
    ...
    routing {
        ...
        get("/dashboard") {
            val html = """
                <html>
                <head>
                    <title>Directo Server Dashboard</title>
                    <style>
                        body { font-family: 'Segoe UI', sans-serif; padding: 40px; background: #f0f2f5; color: #1c1e21; display: flex; gap: 20px; }
                ...
                try {
                    for (frame in incoming) {
                        if (frame is Frame.Text) {
                            ...
                            targets.forEach { it.session.send(text) }
                        }
                    }
                } catch (e: Exception) {
